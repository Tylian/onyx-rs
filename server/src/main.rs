...
use onyx_common::{
    network::{
        AreaData, ChatMessage, ClientId, ClientMessage, Map as NetworkMap,
...
impl GameServer {
    ...
    pub fn save_map(&self, name: &str) -> anyhow::Result<()> {
        let map =
            self.maps.get(name).ok_or_else(|| anyhow!("map doesn't exist"))?;
    ...
    fn handle_message(&mut self, client_id: ClientId, message: ClientMessage) {
        ...
        match message {
            ClientMessage::Hello(name, sprite) => {
                ...
                self.queue(Message::to(
                    client_id,
                    ServerMessage::Message(ChatMessage::Server(
                        "Welcome to Game™!".to_owned(),
                    )),
                ));

                // Send join message
                let welcome = ServerMessage::Message(ChatMessage::Server(
                    format!("{} has joined the game.", &player.name),
                ));
            ...
            ClientMessage::Message(text) => {
                if let Some(player) = self.players.get(&client_id).unwrap() {
                    ...
                    let packet =
                        ServerMessage::Message(ChatMessage::Say(full_text));
            ...
            ClientMessage::Move { position, direction, velocity } => {
                if let Some(player) = self.players.get_mut(&client_id).unwrap()
                {
                    ...
                    player.tween = velocity.map(|v| Tween {
                        velocity: v.into(),
                        last_update: self.time,
                    });
                    let packet = ServerMessage::PlayerMoved {
                        client_id,
                        position,
                        direction,
                        velocity,
        ...
        loop {
            ...
            while let Some(signal) = self.try_recv() {
                match signal {
                    ...
                    NetworkSignal::Connected(client_id) => {
    ...
    fn update_players(&mut self) {
        ...
        for (id, player) in &mut self.players {
            ...
            if let Some(tween) = player.tween.as_mut() {
                let offset = tween.velocity
                    * (self.time - tween.last_update).as_secs_f32();
                ...
                let valid = map_box.contains_box(&sprite)
                    && !map.areas.iter().any(|attrib| {
                        let box2d = Rect::new(
                            attrib.position.into(),
                            attrib.size.into(),
                        )
                        .to_box2d();
                        attrib.data == AreaData::Blocked
                            && box2d.intersects(&sprite)
                    });
                ...
                match &attrib.data {
                    AreaData::Log(message) => {
                        ...
                        if box2d.intersects(&sprite)
                            && player.last_message.elapsed()
                                > Duration::from_secs(1)
                        {
                            let message = ChatMessage::Server(message.clone());
                            packets.push(Message::to(
                                *id,
