...
impl MapEditor {
        ...
        ui.horizontal(|ui| {
            ui.group(|ui| {
                ui.vertical(|ui| {
                    ...
                    if response.clicked() {
                        self.zone_data = ZoneData::Warp(MapId::default(), Point2D::zero(), None);
                    }
                });
            });
    ...
    pub fn show_tools_tab(&mut self, ui: &mut Ui) {
        ...
        Grid::new("resize").num_columns(2).show(ui, |ui| {
            ...
            ui.add_enabled_ui(shift, |ui| {
                ...
                if button.clicked() {
                    self.wants = Some(Wants::Resize(size2(self.new_width, self.new_height)));
                }
            });
        });
    ...
    pub fn tile(&self) -> Tile {
        Tile {
            texture: UVec2::new(
                (self.tile_picker.x / TILE_SIZE) as u32,
                (self.tile_picker.y / TILE_SIZE) as u32,
            ),
