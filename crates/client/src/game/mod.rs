...
use crate::{
    ...
    network::Network, utils::{RectExt, rect, draw_text_shadow}, state::{UpdateContext, DrawContext, SetupContext, EventContext},
};
use notan::{
    draw::*,
...
use self::{camera::Camera, ui::{ChatWindow, MapEditor, Wants, Tab}};
...
impl GameState {
    ...
    fn update_network(&mut self, ctx: &mut UpdateContext) {
    ...
    fn handle_message(&mut self, message: ServerPacket, ctx: &mut UpdateContext) {
        match &message {
